...
def binDCT_C(input_vector):

    if len(input_vector) != 8:
        raise ValueError("Input vector must be of length 8")
    
    # Input vector
    x = np.array(input_vector, dtype=np.float32)
    
    # Butterfly structure computations
    # Stage 1: Basic addition and subtraction
    s0 = x[0] + x[7]
    s1 = x[1] + x[6]
    s2 = x[2] + x[5]
    s3 = x[3] + x[4]
    ...
    # Stage 2: Combine partial sums and apply weights
    t0 = s0 + s3  # First level DC computation
    t1 = s1 + s2  # Combine low-frequency components
    t2 = s0 - s3  # Difference for next levels
    t3 = s1 - s2  # Difference for next levels
    
    # Stage 3: Weighting and recombination
    X0 = t0 * (1/2)  # DC coefficient (scaled)
    X4 = t1 * (1/2)  # AC coefficient (scaled)
    
    # Higher frequency components
    X2 = t2 * (3/8) + t3 * (5/8)
    X6 = t2 * (5/8) - t3 * (3/8)
    
    # Combine higher-order differences for t5, t6, t7
    t5 = s4 + s7
    t6 = s5 + s6
    t7 = s4 - s7
    ...
    return np.array([X0, X4, X6, X2, X7, X3, X5, X1], dtype=np.float32)
...
def binDCT_2D(block):
    ...
    dct_rows = np.zeros_like(block, dtype=np.float32)
    for i in range(8):
        dct_rows[i, :] = binDCT_C(block[i, :])
    
    # Apply 1D DCT to each column of the intermediate result
    dct_result = np.zeros_like(block, dtype=np.float32)
    for j in range(8):
        dct_result[:, j] = binDCT_C(dct_rows[:, j])
...
def dct_2d(block):
    ...
    for u in range(h):
        for v in range(w):
            k=0.25*(((delta(u)*delta(v))))
            for i in range(h):
                for j in range(w):
                    F[u,v]+=k*block[i,j]*np.cos((((2*i)+1)*u*np.pi)/16)*np.cos((((2*j)+1)*v*np.pi)/16)
    return F
...
def main():
    ...
    for i in range(0, heightImg_, 8):
        for j in range(0, withImg_, 8):
            ...
            Y_DCTMatrix = dct_2d(Y_matrix[i:i + 8, j:j + 8])
            Cb_DCTMatrix = dct_2d(Cb_matrix[i:i + 8, j:j + 8])
            Cr_DCTMatrix =dct_2d(Cr_matrix[i:i + 8, j:j + 8])
            # quantization
